...
function addRowToInvoiceItemsTable(
    ...
    var hiddenItemId =
        ...
        '">';
    var inputItemName =
        `<label class="form-label mb-0" role="button">${recordObject.name}</label> ` +
        itemInfoIcon(recordObject.tracking_type);
    inputItemName +=
        recordObject.brand_name !== undefined
    ...
    var stockInUnit =
        '<input type="text" disabled name="stockInUnit[' +
        currentRowId +
        ']" class="form-control" value="' +
        recordObject.stock_in_unit +
        '">';
    ...
    var adjustmentTypeSelectionBox = generateAdjustmentTypeSelectionBox(
        currentRowId,
        recordObject.adjustment_type || "increase"
    );

    /*Keeping the Scheduled Job Records*/
    var removeClass = !recordObject.assigned_user_id ? "remove" : "";
    var inputDeleteButton =
        '<button type="button" class="btn btn-outline-danger remove"><i class="bx bx-trash me-0"></i></button>';
    ...
    newRow.append(
        `<td class="${!itemSettings.enable_serial_tracking ? "d-none" : ""}">` +
            serialTracking +
            hiddenSerialNumbers +
            "</td>"
    );
    newRow.append(
        `<td class="${!itemSettings.enable_batch_tracking ? "d-none" : ""}">` +
            inputBatchNumber +
            "</td>"
    );
    newRow.append(
        `<td class="${!itemSettings.enable_mfg_date ? "d-none" : ""}">` +
            mfgDate +
            "</td>"
    );
    newRow.append(
        `<td class="${!itemSettings.enable_exp_date ? "d-none" : ""}">` +
            expDate +
            "</td>"
    );
    newRow.append(
        `<td class="${!itemSettings.enable_model ? "d-none" : ""}">` +
            modelNo +
            "</td>"
    );
    newRow.append(
 ...
 */
function generateAdjustmentTypeSelectionBox(
    currentRowId,
    selectedType = "increase"
) {
    const options = `
            <option value="increase" ${
                selectedType === "increase" ? "selected" : ""
            }>Increase</option>
            <option value="decrease" ${
                selectedType === "decrease" ? "selected" : ""
            }>Decrease</option>
        `;
...
function afterRemoveFunctions() {
    ...
    // ✅ If no rows left, restore default "No items" row
    if ($("#invoiceItemsTable tbody tr").length === 0) {
        const totalColumns = $(
            "#invoiceItemsTable > thead > tr:first > th"
        ).not(".d-none").length;
        $("#invoiceItemsTable tbody").append(`
            <tr>
                <td colspan="${totalColumns}" class="text-center fw-light fst-italic default-row">
...
$(document).ready(function () {
     ...
     * */
    var totalColumns = $("#invoiceItemsTable > thead > tr:first > th").not(
        ".d-none"
    ).length;
    // "Total" should span all except last 2 (Quantity + Action)
    $(".tfoot-first-td").attr("colspan", totalColumns - 2);
...
$(document).on("focus", 'input[name^="batch_no["]', function () {
    ...
    const warehouseId =
        $(`input[name="warehouse_id[${rowIndex}]"]`).val() ||
        currentWarehouse.val();
    ...
    $input
        .autocomplete({
            ...
            select: function (event, ui) {
                if (ui.item) {
                    $(`input[name="mfg_date[${rowIndex}]"]`).val(
                        ui.item.mfg_date || ""
                    );
                    $(`input[name="exp_date[${rowIndex}]"]`).val(
                        ui.item.exp_date || ""
                    );
                    $(`input[name="model_no[${rowIndex}]"]`).val(
                        ui.item.model_no || ""
                    );
                    $(`input[name="color[${rowIndex}]"]`).val(
                        ui.item.color || ""
                    );
                    $(`input[name="size[${rowIndex}]"]`).val(
                        ui.item.size || ""
                    );
                }
            },
                ...
                `
                    <div style="display:table; width:100%;">
                        <div style="display:table-row;">
                            <div style="display:table-cell; padding:6px 8px; width:18%;">${
                                item.value || ""
                            }</div>
                            <div style="display:table-cell; padding:6px 8px; width:14%;">${
                                item.mfg_date || ""
                            }</div>
                            <div style="display:table-cell; padding:6px 8px; width:14%;">${
                                item.exp_date || ""
                            ...
                            <div style="display:table-cell; padding:6px 8px; width:12%; text-align:right; color:${
                                parseFloat(item.available_stock) > 0
                                    ? "#000"
                                    : "#dc3545"
                            };">${item.available_stock || "N/A"}</div>
                        </div>
                    </div>
                `
            )
...
function setupAutocomplete(inputNamePrefix, requiredField) {
    $(document).on("focus", `input[name^="${inputNamePrefix}["]`, function () {
         ...
         */
        const trackingType = $input
            .closest("tr")
            .find('input[id^="tracking_type["]')
            .val();
        if (trackingType !== "batch") {
