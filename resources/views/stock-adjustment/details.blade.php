...
@section('content')
    ...
    <div class="page-wrapper">
        <div class="page-content">
            <x-breadcrumb :langArray="['item.stock', 'warehouse.stock_adjustment', 'app.print']" />
            <div class="row">
                            ...
                            <div class="toolbar hidden-print">
                                <div class="text-end">
                                    ...
                                    @can(['stock_adjustment.edit'])
                                        <a href="{{ route('stock_adjustment.edit', ['id' => $adjustment->id]) }}"
                                            class="btn btn-outline-primary"><i class="bx bx-edit"></i>{{ __('app.edit') }}</a>
                                    @endcan

                                    <a href="{{ route('stock_adjustment.print', ['id' => $adjustment->id]) }}"
                                        target="_blank" class="btn btn-outline-secondary px-4"><i
                                            class="bx bx-printer mr-1"></i>{{ __('app.print') }}</a>

                                    <a href="{{ route('stock_adjustment.pdf', ['id' => $adjustment->id]) }}" target="_blank"
                                        class="btn btn-outline-danger px-4"><i
                                            class="bx bxs-file-pdf mr-1"></i>{{ __('app.pdf') }}</a>
                                    ...
                                    <div class="min-width-600">
                                        <header>
                                            <div class="row">
                                                <div class="col">
                                                    <a href="javascript:;">
                                                        <img src={{ '/company/getimage/' . app('company')['colored_logo'] }}
                                                            width="80" alt="" />
                                                    </a>
                                                </div>
                                            ...
                                            <div class="row contacts">
                                                <div class="col invoice-to">
                                                    <div class="text-gray-light fw-bold text-uppercase">
                                                        {{ __('warehouse.stock_adjustment') }}</div>
                                                </div>

                                                <div class="col invoice-details">
                                                    <h1 class="invoice-id">#{{ $adjustment->adjustment_code }}</h1>
                                                    <div class="date">{{ __('app.date') }}:
                                                        {{ $adjustment->formatted_adjustment_date }}</div>
                                                    @if ($adjustment->reference_no)
                                                        <div class="date">{{ __('app.reference_no') }}:
                                                            {{ $adjustment->reference_no }}</div>
                                                    @endif
                                            ...
                                            <table id="printInvoice">
                                                <thead>
                                                    <tr class="text-uppercase">
                                                        ...
                                                        <th
                                                            class="text-left {{ !app('company')['show_hsn'] ? 'd-none' : '' }}">
                                                            {{ __('item.hsn') }}</th>
                                                ...
                                                <tfoot>
                                                    @php
                                                        $totalQuantity = $adjustment->itemTransaction->sum(function (
                                                            $transaction,
                                                        ) {
                                                    ...
                                                    @endphp
                                                    <tr>
                                                        <td colspan="{{ $columnCount }}" class="tfoot-first-td">
                                                            {{ __('app.total') }}</td>
                                                        <td class="text-start">
                                                            {{ $formatNumber->formatQuantity($totalQuantity) }}</td>
                                                    </tr>
